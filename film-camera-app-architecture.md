...
Processing always starts by normalizing orientation and choosing the full/preview render size, then walks the recipe's stage list in order, renders to finite sRGB RGBA, and encodes through Image I/O (JPEG for developed stills, copying the capture's EXIF/TIFF metadata with the orientation reset to upright and no embedded thumbnail). The legacy pipeline order (still used by Night, Cinema, and legacy-original) is tone curve, colour response, bloom/halation, softness, chromatic aberration, seeded grain, optional seeded light leak, vignette, and the persisted date stamp. The 1998 recipe (recipe version 4) replaces the parametric colour grade with a `filmResponse` stage: a `FilmResponseStage` fitted offline by `tools/film-response-fit/` from iPhone originals aligned to reference shots of the same scenes (crosstalk matrix in linear light, per-channel monotone tone curves, tone-dependent chroma, and per-hue-band chroma/rotation/lightness in OKLab), baked into the same 32³ `CIColorCube` as the parametric grade so stills and video share it; `FilmResponseModelTests` pins the Swift port to Python-generated probes. Around it the recipe keeps its retuned optics: a wide fixed pink halation, Gaussian rather than radial-zoom softness, the seven-segment date stamp drawn *before* chromatic aberration so the LED digits pick up the same red/blue fringe as the scene, an unseeded (deterministic) lateral-free chromatic aberration, film grain, orange light leaks restricted to the top/right edges with a rendered alpha cap, and a soft vignette. Version 1–3 manifests still decode and re-develop through their persisted `colorGrade`. Only the first light-leak stage in a recipe consumes the persisted leak decision. Thumbnails are a separate deterministic downsample of the processed image; they do not alter the full-resolution export.
