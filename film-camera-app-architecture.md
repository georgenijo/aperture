...
Processing always starts by normalizing orientation and choosing the full/preview render size, then walks the recipe's stage list in order, renders to finite sRGB RGBA, and encodes through Image I/O (JPEG for developed stills, copying the capture's EXIF/TIFF metadata with the orientation reset to upright and no embedded thumbnail). The legacy pipeline order (still used by Night, Cinema, and legacy-original) is tone curve, colour response, bloom/halation, softness, chromatic aberration, seeded grain, optional seeded light leak, vignette, and the persisted date stamp. The 1998 recipe (recipe version 3) reorders and retunes those stages toward the Huji look: a fixed pink halation tint, Gaussian rather than radial-zoom softness, the seven-segment date stamp drawn *before* chromatic aberration so the LED digits pick up the same red/blue fringe as the scene, an unseeded (deterministic) lateral-free chromatic aberration, and orange light leaks restricted to the top/right edges with a rendered alpha cap. Only the first light-leak stage in a recipe consumes the persisted leak decision. Thumbnails are a separate deterministic downsample of the processed image; they do not alter the full-resolution export.
